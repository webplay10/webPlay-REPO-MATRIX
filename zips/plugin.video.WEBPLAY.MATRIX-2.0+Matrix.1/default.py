...
##CONFIGURAÇÕES
####  TITULO DO MENU  #################################################################
title_menu = "[COLOR azure]WEB[/COLOR][B][COLOR red]PLAY MATRIX[/COLOR][/B]"
###  DESCRIÇÃO DO ADDON ###############################################################
...
menu_pesquisar = '[COLOR orangered][B]PESQUISAR...[/B][/COLOR]'
...
## MENU CONFIGURAÇÕES
menu_configuracoes = "[B][COLOR orangered]CONFIGURAÇÕES[/COLOR][/B]"
...
## FAVORITOS
menu_favoritos = '[B][COLOR orangered]|[/COLOR][COLOR orangered]FAVORITOS[/COLOR][COLOR orangered]|[/COLOR][/B]'
...
#### MENU VIP ################################################################
titulo_vip = "[B][COLOR orangered]WEBPLAY[/COLOR] - [COLOR orangered](VIP)[/COLOR][/B]"
...
vip_dialogo = "[B][COLOR orangered]CONFIGURE SEU ACESSO[/COLOR][/B]"
##SERIVODR VIP
url_server_vip1 = ''
url_server_vip = ''
...
dialog_playlist = '[B][COLOR orangered]Escolha uma opção:[/COLOR][/B]'
...
url_origem = 'https://raw.githubusercontent.com/webplay10/WEBPLAY/master/Categorias'
try:
    ...
    if exists == False:
        ...
        if ps == p_file_b64_decode:
            ...
            if ps2 != '':
                ...
                p_file.close()
                xbmcgui.Dialog().ok('[B][COLOR orangered]AVISO[/COLOR][/B]','A Senha foi alterada com sucesso!')
            else:
                xbmcgui.Dialog().ok('[B][COLOR orangered]AVISO[/COLOR][/B]','Não foi possivel alterar a senha!')
        else:
            xbmcgui.Dialog().ok('[B][COLOR orangered]AVISO[/COLOR][/B]','Senha invalida!, se não alterou utilize a senha padrão')
    else:
        p_file = open(arquivo,'r+')
        p_file_read = p_file.read()
        p_file_b64_decode = base64.b64decode(p_file_read).decode('utf-8')
...
def resolver(link,regex):
    #xbmcgui.Dialog().ok('[B][COLOR orangered]AVISO[/COLOR][/B]',str(regex))
...
def getFavorites():
    try:
        ...
        if int(total) > 0:
            ...
            xbmcgui.Dialog().ok('[B][COLOR orangered]AVISO[/COLOR][/B]','Nada Adicionado nos Favoritos')
    except:
        pass
...
def Pesquisa():
    ...
    addDir('[COLOR orangered][B]PESQUISAR NOVAMENTE...[/B][/COLOR]','',7,thumb_pesquisar,fanart_pesquisar,desc_pesquisa,'','','','','','','','','','','','','','','','')
    try:
...
def kill_kodi():
    dialog = xbmcgui.Dialog()
    link = dialog.select('[B][COLOR orangered]FINALIZANDO INSTALAÇÃO DO ELEMENTUM[/COLOR][/B]', ['[COLOR orangered]SIM POR FAVOR! [/COLOR][COLOR orange]|[/COLOR] [COLOR lime]FORÇAR FECHAR[/COLOR]','[COLOR orangered]NÃO POR FAVOR! [/COLOR][COLOR orange]|[/COLOR] [COLOR red]NÃO CANCELAR[/COLOR]'])    
...
def init_SKindex(msg):
   ...
   if status_mensagem1 == 'true':
    dialog = xbmcgui.Dialog()
    link = dialog.select('SEJAM [COLOR  azure] BEM - VINDOS AO [/COLOR][B][COLOR azure] WEB[/COLOR][COLOR red]PLAY![/COLOR][/B]', ['[B][COLOR azure] WEB[/COLOR][COLOR red]PLAY:[/COLOR][/B] MERCADO PAGO', '[B][COLOR azure] WEB[/COLOR][COLOR red]PLAY:[/COLOR][/B] PAGSEGURO', '[B][COLOR azure] WEB[/COLOR][COLOR red]PLAY:[/COLOR][/B] WHATSAPP', '[B][COLOR azure] WEB[/COLOR][COLOR red]PLAY:[/COLOR][/B] FACEBOOK', '[B][COLOR azure]ACESSAR [/COLOR] [COLOR red] AQUI[/COLOR] [/B]'])    
...
#Configurações
    ...
    xbmcgui.Dialog().ok('[B][COLOR red]AVISO IMPORTANTE![/COLOR][/B]','[B][COLOR orange]|[/COLOR][/B][COLOR orangered] POR FAVOR SAIR DO ADD-ON E ENTRE NOVAMENTE,[B] [COLOR orange]|[/COLOR][/B] PARA ATUALIZAR AS CONFIGURAÇÕES![B] [COLOR orange]|[/COLOR][/B]')
    xbmc.executebuiltin("XBMC.Container.Refresh()")
...
#    Pesquisa()
    ...
    xbmcaddon.Addon().openSettings()
    xbmcgui.Dialog().ok('[B][COLOR orangered]AVISO[/COLOR][/B]','FECHE O KODI E ABRA NOVAMENTE PARA ATUALIZAR AS CONFIGURAÇÕES')
    xbmc.executebuiltin("XBMC.Container.Refresh")
